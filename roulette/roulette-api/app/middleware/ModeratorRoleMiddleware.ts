...
/**
 * ModeratorRole Middleware Options
*/
export interface ModeratorRoleMiddlewareOptions {
  securityService: SecurityService
}

/**
 * ModeratorRole middleware
 */
@Middleware({ alias: 'moderator', priority: 20 })
export class ModeratorRoleMiddleware implements IMiddleware<IncomingHttpEvent, OutgoingHttpResponse> {
  private readonly securityService: SecurityService

  /**
   * Create a new instance of ModeratorRole Middleware
   *
   * @param options - The options to create the middleware
   */
  constructor ({ securityService }: ModeratorRoleMiddlewareOptions) {
  ...
  async handle (
    ...
    if (!this.securityService.isAdmin(event.getUser<User>()) && !this.securityService.hasRole(event.getUser<User>(), 'moderator')) {
