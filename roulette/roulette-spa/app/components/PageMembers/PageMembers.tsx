...
export const PageMembers: React.FC<PageMembersProps> = ({ members, currentUser }) => {
  return (
    <div className="bg-white/5 rounded-xl p-6 text-white">
      ...
      {members.length > 0 ? (<ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4 mt-6">
        {members.map((member) => (
          <li key={member.uuid} className='bg-[#123840] border border-neutral-800 rounded-lg p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-md'>
            <div className='flex items-center gap-4'>
              ...
              <div>
                <p className='text-lg font-semibold text-white capitalize'>{member.name}</p>
                {currentUser.isAdmin  && <p className='text-sm text-white/80 capitalize'>@{member.user?.fullname}</p>}
                {member.isPresent ? <p className={`text-xs font-medium mt-1 ${!member.isLate ? 'text-green-400' : 'text-yellow-500'}`}>
                  {member.isLate ? 'Présent mais en retard' : "Présent et à l'heure"}
                </p> : <p className='text-xs font-medium mt-1 text-red-500'>Absent</p>}
              </div>
            </div>
      
            <div>
              {member.role === 'captain' ? (
                <span className="ml-auto text-xs px-2 py-1 bg-orange-600/80 rounded-full text-white flex items-center">
